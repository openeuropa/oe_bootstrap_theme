...
const buildColorScheme = (entry, dest, options) => {
  ...
  const importsPath = options.importsFile
    ? path.resolve(options.importsFile)
    : path.resolve(
        ...(options.includePaths || []),
        "@openeuropa/bcl-theme-default/src/scss/color-scheme.scss",
      );

  const generatorPath = options.generatorFile
    ? path.resolve(options.generatorFile)
    : path.resolve(
        ...(options.includePaths || []),
    ...
    .use(
      prefixer({
        ...
        transform: (prefix, selector, prefixedSelector) =>
          prefix ? prefixedSelector : selector,
      }),
    )
